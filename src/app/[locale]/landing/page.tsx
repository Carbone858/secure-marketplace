...
export default async function LandingPage({ params: { locale } }: { params: { locale: string } }) {
  ...
  return (
    <div className="w-full overflow-x-hidden bg-background min-h-screen">
      ...
      <section className="relative min-h-[90vh] flex items-center py-24 px-4 md:px-8 overflow-hidden bg-slate-950">
        ...
        <div className="container mx-auto relative z-10">
          <div className="max-w-2xl space-y-7">
            {/* Branding Pill */}
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/10 border border-white/20 backdrop-blur-sm text-white font-semibold text-sm">
              <span className="w-2 h-2 rounded-full bg-green-400 animate-pulse" />
              <span>{t('hero.title')}</span>
            </div>

            {/* Headline */}
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-black tracking-tight text-white leading-[1.15] drop-shadow-lg">
              {t('hero.subtitle')}
            </h1>

            {/* Sub-headline */}
            <p className="text-lg md:text-xl text-slate-200 leading-relaxed font-normal max-w-xl">
              {t('hero.description')}
            </p>
            ...
            <div className="flex flex-col sm:flex-row gap-4 pt-2">
              <Button
                asChild
                size="lg"
                className="bg-primary hover:bg-primary/90 text-white font-bold text-lg px-8 py-6 rounded-xl shadow-2xl shadow-primary/40 h-auto transition-transform hover:-translate-y-0.5"
              >
              ...
              <Button
                ...
                variant="outline"
                size="lg"
                className="bg-white/10 hover:bg-white/20 border-white/30 text-white font-bold text-lg px-8 py-6 rounded-xl h-auto backdrop-blur-sm transition-transform hover:-translate-y-0.5"
              >
            ...
            <div className="flex flex-wrap gap-3 pt-2">
              ...
              <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-white/10 border border-white/10 text-white/80 text-sm font-medium backdrop-blur-sm">
                ...
                {locale === 'ar' ? 'جميع المحافظات' : 'All Governorates'}
              </span>
            </div>
          </div>
        </div>
